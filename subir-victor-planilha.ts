/**
 * Cadastra na Biblioteca (planilha) os criativos 9x16 do lote "Bready" do Victor,
 * direto da pasta 9x16 do Drive (Body1/Body2/Body3, h1–h9 cada = 27 criativos).
 * Sem cta no naming (vv-H{n}B{n}-Bready-Victor-9x16). Ordem body→hook, TP sequencial (max+1).
 * Só 9x16 entra na planilha (padrão de sempre).
 *
 *   npx tsx subir-victor-planilha.ts        # DRY
 *   npx tsx subir-victor-planilha.ts --go   # grava
 */
...
const ROOT = "1FVT_Z_c-Ob0NwEQydsB7Svn4x3043HCz"; // Victor › 9x16 (Body1/2/3)
const PERSONAGEM = "Victor";
const TEMA = "Bready";
...
(async () => {
  ...
  console.log(`Victor "Bready" 9x16 no Drive: ${files.length} arquivo(s) · ${toReg.length} a cadastrar (ordem body→hook)`);
  console.log(`Próximo TP: ${startTp}  →  iria de ${startTp} até TP${parseInt(startTp.replace("TP", ""), 10) + toReg.length - 1}\n`);
  for (const f of toReg) { const p = parse(f.name); console.log(`  • b${p.b} h${p.h}  ${f.name}`); }
  ...
  for (const f of toReg) {
    ...
    const row = await createCreative({
      ...
      tipoAd: "Vídeo",
      observacao: `Tema: ${TEMA} | Hook ${p.h} | b${p.b}${p.c ? ` c${p.c}` : ""} | 9x16`,

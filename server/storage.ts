...
export interface IStorage {
  ...
  getGegOverview(filters?: { mesInicio?: string; mesFim?: string; squad?: string; setor?: string }): Promise<import("@shared/schema").GegOverview>;
  getGegHeadcountTrend(filters?: { mesInicio?: string; mesFim?: string; squad?: string; setor?: string }): Promise<import("@shared/schema").GegHeadcountTrend[]>;
  getGegDemissoesBreakdown(filters?: { mesInicio?: string; mesFim?: string; squad?: string; setor?: string }): Promise<import("@shared/schema").GegDemissoesBreakdown[]>;
  getTempoMedioPromocaoPorSquad(filters?: { mesInicio?: string; mesFim?: string }): Promise<import("@shared/schema").TempoMedioPromocaoPorSquad[]>;
}
...
export class MemStorage implements IStorage {
  ...
  async getGegDemissoesBreakdown(filters?: { mesInicio?: string; mesFim?: string; squad?: string; setor?: string }): Promise<import("@shared/schema").GegDemissoesBreakdown[]> {
    throw new Error("Not implemented in MemStorage");
  }

  async getTempoMedioPromocaoPorSquad(filters?: { mesInicio?: string; mesFim?: string }): Promise<import("@shared/schema").TempoMedioPromocaoPorSquad[]> {
    throw new Error("Not implemented in MemStorage");
  }
}
...
export class DbStorage implements IStorage {
  ...
  async getGegOverview(filters?: { mesInicio?: string; mesFim?: string; squad?: string; setor?: string }): Promise<import("@shared/schema").GegOverview> {
    ...
    if (filters?.mesInicio && filters?.mesFim) {
      conditions.push(sql`(admissao BETWEEN ${filters.mesInicio}::date AND ${filters.mesFim}::date OR demissao BETWEEN ${filters.mesInicio}::date AND ${filters.mesFim}::date OR (admissao <= ${filters.mesInicio}::date AND (demissao IS NULL OR demissao >= ${filters.mesFim}::date)))`);
    }
    
    if (filters?.squad) {
      conditions.push(sql`squad = ${filters.squad}`);
    }
    
    if (filters?.setor) {
      conditions.push(sql`setor = ${filters.setor}`);
    }

    const whereClause = conditions.length > 0 ? sql`WHERE ${sql.join(conditions, sql` AND `)}` : sql``;

    const headcountAtualResult = await db.execute(sql`
      SELECT COUNT(*) as count
      FROM ${schema.rhPessoal}
      WHERE demissao IS NULL
      ${filters?.squad ? sql`AND squad = ${filters.squad}` : sql``}
      ${filters?.setor ? sql`AND setor = ${filters.setor}` : sql``}
    `);
    const headcountAtual = parseInt((headcountAtualResult.rows[0] as any).count || '0');

    let admissoesPeriodo = 0;
    let demissoesPeriodo = 0;
    let headcountInicialPeriodo = headcountAtual;
    
    if (filters?.mesInicio && filters?.mesFim) {
      const admiResult = await db.execute(sql`
        SELECT COUNT(*) as count
        FROM ${schema.rhPessoal}
        WHERE admissao BETWEEN ${filters.mesInicio}::date AND ${filters.mesFim}::date
        ${filters?.squad ? sql`AND squad = ${filters.squad}` : sql``}
        ${filters?.setor ? sql`AND setor = ${filters.setor}` : sql``}
      `);
      admissoesPeriodo = parseInt((admiResult.rows[0] as any).count || '0');

      const demiResult = await db.execute(sql`
        SELECT COUNT(*) as count
        FROM ${schema.rhPessoal}
        WHERE demissao BETWEEN ${filters.mesInicio}::date AND ${filters.mesFim}::date
        ${filters?.squad ? sql`AND squad = ${filters.squad}` : sql``}
        ${filters?.setor ? sql`AND setor = ${filters.setor}` : sql``}
      `);
      demissoesPeriodo = parseInt((demiResult.rows[0] as any).count || '0');

      const headcountInicialResult = await db.execute(sql`
        SELECT COUNT(*) as count
        FROM ${schema.rhPessoal}
        WHERE admissao < ${filters.mesInicio}::date
          AND (demissao IS NULL OR demissao >= ${filters.mesInicio}::date)
        ${filters?.squad ? sql`AND squad = ${filters.squad}` : sql``}
        ${filters?.setor ? sql`AND setor = ${filters.setor}` : sql``}
      `);
    ...
    const turnoverPeriodo = headcountMedio > 0 ? (demissoesPeriodo / headcountMedio) * 100 : 0;
    const headcountLiquidoPeriodo = admissoesPeriodo - demissoesPeriodo;
    const taxaRetencao = headcountMedio > 0 ? ((headcountMedio - demissoesPeriodo) / headcountMedio) * 100 : 100;

    const tempoMedioAtivoResult = await db.execute(sql`
      SELECT AVG(EXTRACT(DAYS FROM (COALESCE(demissao, CURRENT_DATE) - admissao)) / 30.0) as tempo_medio
      FROM ${schema.rhPessoal}
      WHERE admissao IS NOT NULL AND demissao IS NULL
      ${filters?.squad ? sql`AND squad = ${filters.squad}` : sql``}
      ${filters?.setor ? sql`AND setor = ${filters.setor}` : sql``}
    `);
    const tempoMedioAtivoMeses = parseFloat((tempoMedioAtivoResult.rows[0] as any)?.tempo_medio || '0');

    const mesAtual = now.getMonth() + 1;
    const anoAtual = now.getFullYear();
    const aniversariantesResult = await db.execute(sql`
      SELECT id, nome, aniversario, squad, cargo,
             EXTRACT(DAYS FROM (
               DATE_PART('year', AGE(CURRENT_DATE, aniversario))::int * INTERVAL '1 year' + 
               DATE_PART('month', AGE(CURRENT_DATE, aniversario))::int * INTERVAL '1 month' + 
               DATE_PART('day', AGE(CURRENT_DATE, aniversario))::int * INTERVAL '1 day'
             )) as dias_restantes
      FROM ${schema.rhPessoal}
    ...
    const aniversariantes: import("@shared/schema").Aniversariante[] = (aniversariantesResult.rows as any[]).map(row => ({
      id: row.id,
      nome: row.nome,
      aniversario: new Date(row.aniversario),
      diasRestantes: parseInt(row.dias_restantes || '0'),
      squad: row.squad,
      cargo: row.cargo,
    }));

    const proximosAniversariosResult = await db.execute(sql`
      SELECT id, nome, admissao, squad, cargo,
             EXTRACT(YEAR FROM AGE(CURRENT_DATE, admissao)) as anos_completos,
             EXTRACT(DAYS FROM (
               (DATE_TRUNC('year', admissao) + (EXTRACT(YEAR FROM AGE(CURRENT_DATE, admissao)) + 1) * INTERVAL '1 year') - CURRENT_DATE
             )) as dias_restantes
      FROM ${schema.rhPessoal}
    ...
    const proximosAniversariosEmpresa: import("@shared/schema").ProximoAniversarioEmpresa[] = (proximosAniversariosResult.rows as any[]).map(row => ({
      id: row.id,
      nome: row.nome,
      admissao: new Date(row.admissao),
      anosCompletos: parseInt(row.anos_completos || '0'),
      diasRestantes: parseInt(row.dias_restantes || '0'),
      squad: row.squad,
      cargo: row.cargo,
    }));
    ...
    const result = {
      ...
      admissoesPeriodo,
      demissoesPeriodo,
      tempoMedioAtivoMeses,
      headcountLiquidoPeriodo,
      taxaRetencao,
      aniversariantes,
  ...
  async getGegHeadcountTrend(filters?: { mesInicio?: string; mesFim?: string; squad?: string; setor?: string }): Promise<import("@shared/schema").GegHeadcountTrend[]> {
    const mesInicio = filters?.mesInicio || '2024-01-01';
    const mesFim = filters?.mesFim || new Date().toISOString().split('T')[0];

    const result = await db.execute(sql`
      WITH RECURSIVE meses AS (
        SELECT DATE_TRUNC('month', ${mesInicio}::date)::date as mes
        UNION ALL
        SELECT (mes + INTERVAL '1 month')::date
        FROM meses
        WHERE mes < DATE_TRUNC('month', ${mesFim}::date)::date
      ),
      admissoes_por_mes AS (
        SELECT 
          DATE_TRUNC('month', admissao)::date as mes,
          COUNT(*) as admissoes
        FROM ${schema.rhPessoal}
        WHERE admissao IS NOT NULL
          AND admissao BETWEEN ${mesInicio}::date AND ${mesFim}::date
          ${filters?.squad ? sql`AND squad = ${filters.squad}` : sql``}
          ${filters?.setor ? sql`AND setor = ${filters.setor}` : sql``}
        GROUP BY DATE_TRUNC('month', admissao)
      ),
      demissoes_por_mes AS (
        SELECT 
          DATE_TRUNC('month', demissao)::date as mes,
          COUNT(*) as demissoes
        FROM ${schema.rhPessoal}
        WHERE demissao IS NOT NULL
          AND demissao BETWEEN ${mesInicio}::date AND ${mesFim}::date
          ${filters?.squad ? sql`AND squad = ${filters.squad}` : sql``}
          ${filters?.setor ? sql`AND setor = ${filters.setor}` : sql``}
        GROUP BY DATE_TRUNC('month', demissao)
      )
      SELECT 
        TO_CHAR(m.mes, 'MM/YYYY') as mes,
        COALESCE(a.admissoes, 0) as admissoes,
        COALESCE(d.demissoes, 0) as demissoes
      FROM meses m
      LEFT JOIN admissoes_por_mes a ON m.mes = a.mes
      LEFT JOIN demissoes_por_mes d ON m.mes = d.mes
      ORDER BY m.mes
    `);

    let headcountAcumulado = 0;
    
    const headcountInicialResult = await db.execute(sql`
      SELECT COUNT(*) as count
      FROM ${schema.rhPessoal}
      WHERE admissao < ${mesInicio}::date
        AND (demissao IS NULL OR demissao >= ${mesInicio}::date)
        ${filters?.squad ? sql`AND squad = ${filters.squad}` : sql``}
        ${filters?.setor ? sql`AND setor = ${filters.setor}` : sql``}
    `);
    headcountAcumulado = parseInt((headcountInicialResult.rows[0] as any).count || '0');

    return (result.rows as any[]).map(row => {
      const admissoes = parseInt(row.admissoes || '0');
      const demissoes = parseInt(row.demissoes || '0');
      headcountAcumulado += admissoes - demissoes;
      
      return {
        mes: row.mes,
        headcount: headcountAcumulado,
        admissoes,
        demissoes,
      };
    });
  ...
  async getGegDemissoesBreakdown(filters?: { mesInicio?: string; mesFim?: string; squad?: string; setor?: string }): Promise<import("@shared/schema").GegDemissoesBreakdown[]> {
    const conditions: SQL[] = [sql`demissao IS NOT NULL`];
    
    if (filters?.mesInicio && filters?.mesFim) {
      conditions.push(sql`demissao BETWEEN ${filters.mesInicio}::date AND ${filters.mesFim}::date`);
    }
    ...
    const result = await db.execute(sql`
      WITH total_demissoes AS (
        SELECT COUNT(*) as total
        FROM ${schema.rhPessoal}
        ${whereClause}
      )
      SELECT 
        COALESCE(motivo_demissao, 'Não informado') as motivo,
        COUNT(*) as quantidade,
        (COUNT(*) * 100.0 / td.total) as percentual
      FROM ${schema.rhPessoal}, total_demissoes td
      ${whereClause}
      GROUP BY motivo_demissao, td.total
  ...
  async getTempoMedioPromocaoPorSquad(filters?: { mesInicio?: string; mesFim?: string }): Promise<import("@shared/schema").TempoMedioPromocaoPorSquad[]> {
    const conditions: SQL[] = [
      sql`ultimo_aumento IS NOT NULL`,
      sql`admissao IS NOT NULL`,
      sql`demissao IS NULL`,
      sql`squad IS NOT NULL`,
    ];
    
    if (filters?.mesInicio && filters?.mesFim) {
      conditions.push(sql`ultimo_aumento BETWEEN ${filters.mesInicio}::date AND ${filters.mesFim}::date`);
    }
    ...
    const result = await db.execute(sql`
      SELECT 
        squad,
        AVG(EXTRACT(DAYS FROM (ultimo_aumento - admissao)) / 30.0) as tempo_medio_meses,
        COUNT(*) as total_colaboradores
      FROM ${schema.rhPessoal}
      ${whereClause}
      GROUP BY squad
      HAVING COUNT(*) > 0
      ORDER BY tempo_medio_meses
    `);

    return (result.rows as any[]).map(row => ({
      squad: row.squad || 'Sem squad',
      tempoMedioMeses: parseFloat(row.tempo_medio_meses || '0'),

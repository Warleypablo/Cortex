...
async function handleResumo(periodo?: string) {
  ...
  const inadimplentesResult = await db.execute(sql`
    SELECT COUNT(*)::int AS total FROM juridico_clientes
  `);
  const totalInadimplentes = inadimplentesResult.rows[0]?.total ?? 0;

  // KPI: Valor em risco (parcelas vencidas)
  const valorRiscoResult = await db.execute(sql`
    SELECT COALESCE(SUM(valor), 0)::numeric AS total
    FROM "Conta Azul".caz_parcelas
    WHERE status = 'VENCIDO'
  `);
  const valorEmRisco = Number(valorRiscoResult.rows[0]?.total ?? 0);
  ...
  return {
    ...
    porProcedimento: procResult.rows,
...
async function handleProcessos() {
  ...
  // Por natureza
  const naturezaResult = await db.execute(sql`
    SELECT
  ...
  // Lista
  const listaResult = await db.execute(sql`
    SELECT
      id,
  ...
  return { kpis, porNatureza: naturezaResult.rows, porStatus: statusResult.rows, lista: listaResult.rows };
}
...
async function handleContratos() {
  try {
    ...
    return {
      kpis: {
        totalContratos: row?.totalContratos ?? 0,
        contratosAtivos: row?.contratosAtivos ?? 0,
      },
    };
  } catch (err) {

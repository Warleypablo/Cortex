        ...
        for (let m = 1; m < currentMonthNum; m++) {
          ...
          try {
            ...
            const [snapMrr, snapVendas, snapPontual, snapOutras, snapInad, snapImpostos, snapCaixa, snapCsv, snapSga, snapCac, snapCapex, snapIrCsll, snapChurn, snapHeadcount, snapClientes, snapContratos] = await Promise.all([
              ...
              db.execute(sql.raw(`SELECT COALESCE(SUM(valor_r), 0) as churn FROM cortex_core.vw_cup_churn_ajustado WHERE data_solicitacao_encerramento IS NOT NULL AND data_solicitacao_encerramento >= '${mStart}' AND data_solicitacao_encerramento <= '${mEnd}' AND COALESCE(abonar_churn, '') != 'Sim' AND COALESCE(motivo_cancelamento, '') NOT IN ('Inadimplente 1º Mês', 'Não começou', 'Erro na Venda') AND valor_r > 0`)),

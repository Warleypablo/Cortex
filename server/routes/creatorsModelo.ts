  ...
  app.get("/api/creators-modelo/evolucao", async (req, res) => {
    try {
      ...
      const result = await db.execute(sql`
        ...
        -- Vendas pontual = contratos cujo data_inicio cai naquele mês (new bookings).
        pont_subtask_venda AS (
          SELECT DISTINCT ON (id_subtask) to_char(di,'YYYY-MM') AS m_venda, id_task, vp
          FROM snap WHERE vp > 0 AND id_subtask IS NOT NULL
          ORDER BY id_subtask, m
        ),
        pont_venda AS (
          SELECT pv.m_venda AS m, SUM(pv.vp) AS venda
          FROM pont_subtask_venda pv
          JOIN pont_unit pu ON pu.uid = pv.id_task AND pu.m = pv.m_venda
          WHERE ${pontPred}
          GROUP BY pv.m_venda
        )
        SELECT mz.m AS mes,
        ...
        LEFT JOIN pont_venda pv ON pv.m = mz.m
        ORDER BY mz.m
      `);
      ...
      const meses = (result.rows as any[]).map((r) => {
        ...
        return {
          mes: r.mes,
          recorrente: { clientes: Number(r.rec_cli), clientesLt: Number(r.rec_cli), lt: r1(r.rec_lt), ltv: r0(r.rec_ltv), faturamento: r0(r.rec_fat), vendas: null, ticket: recTicket },
          pontual: { clientes: Number(r.pont_cli), clientesLt: Number(r.pont_cli_lt), lt: r1(r.pont_lt), ltv: r0(r.pont_ltv), faturamento: r0(r.pont_fat), vendas: r0(r.pont_venda), ticket: pontTicket },
        };
      });
      res.json({ meses });
    } catch (error) {

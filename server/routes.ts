...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Debug endpoint para verificar dados do caz_parcelas
  app.get("/debug-caz-parcelas", async (req, res) => {
    try {
      // Verificar contagem total
      const countResult = await db.execute(sql`SELECT COUNT(*) as total FROM caz_parcelas`);
      ...
      // Verificar valores únicos de tipo_evento
      const tipoEventoResult = await db.execute(sql`
        SELECT DISTINCT tipo_evento, COUNT(*) as qtd 
        FROM caz_parcelas 
      ...
      const sampleResult = await db.execute(sql`
        SELECT * FROM caz_parcelas LIMIT 5
      `);
      
      res.json({
        totalRegistros: (countResult.rows[0] as any)?.total,
        colunas: columnsResult.rows,
        statusUnicos: statusResult.rows,
        tiposEventoUnicos: tipoEventoResult.rows,
        amostra: sampleResult.rows
      });
    } catch (error) {
      console.error("[debug-caz-parcelas] Error:", error);
      res.status(500).json({ error: String(error) });
    }
  });

  // Debug endpoint para verificar todas as tabelas caz_*
  app.get("/debug-caz-tables", async (req, res) => {
    try {
      const tables = ['caz_bancos', 'caz_parcelas', 'caz_receber', 'caz_pagar', 'caz_clientes', 'caz_categorias'];
      const counts: Record<string, number> = {};
      
      for (const table of tables) {
        const result = await db.execute(sql.raw(`SELECT COUNT(*) as total FROM ${table}`));
        counts[table] = parseInt((result.rows[0] as any)?.total || '0');
      }
      
      res.json({ counts });
    } catch (error) {
      console.error("[debug-caz-tables] Error:", error);
      res.status(500).json({ error: String(error) });
    }
  });

  // Debug endpoint para verificar estrutura de caz_receber e caz_pagar
  app.get("/debug-caz-receber-pagar", async (req, res) => {
    try {
      const receberColumns = await db.execute(sql`
        SELECT column_name, data_type 
        FROM information_schema.columns 
        WHERE table_name = 'caz_receber' 
        ORDER BY ordinal_position
      `);
      
      const pagarColumns = await db.execute(sql`
        SELECT column_name, data_type 
        FROM information_schema.columns 
      ...
      const receberSample = await db.execute(sql`SELECT * FROM caz_receber LIMIT 3`);
      const pagarSample = await db.execute(sql`SELECT * FROM caz_pagar LIMIT 3`);
      
      res.json({
        caz_receber: {
          colunas: receberColumns.rows,
          amostra: receberSample.rows
        },
        caz_pagar: {
          colunas: pagarColumns.rows,
          amostra: pagarSample.rows
        }
      });
    } catch (error) {
      console.error("[debug-caz-receber-pagar] Error:", error);
      res.status(500).json({ error: String(error) });
    }
  });

...
- **Key Features**:
    ...
    - **Service Icons**: Visual service representation system (`client/src/lib/service-icons.tsx`) maps service names from `cup_contratos.servico` to category-specific Lucide icons with color coding. Uses NFD normalization for accent-insensitive matching. Covers 20+ service categories including Performance, Social Media (including "Redes Sociais"), Creators, CRM, E-mail, E-commerce, SEO, Automação, etc. Displayed in `ServiceIcons` component with shadcn Tooltips showing full service name on hover. Limited to 5 visible icons with overflow indicator.
    ...
    - **Contract Management**: Comprehensive tracking from `cup_contratos`, displaying service type, status, squad, and financial values (valorr for recurring, valorp for one-time). Includes status-based color coding. "Responsável" column shows `cup_contratos.responsavel`, "CS" column shows `cup_contratos.csResponsavel`.

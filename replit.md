...
- **Key Features**:
    ...
    - **Contract Management**: Comprehensive tracking from `cup_contratos`, displaying service type (from `produto` column with fallback to `servico`), status, squad, and financial values (valorr for recurring, valorp for one-time). Includes status-based color coding. "Responsável" column shows `cup_contratos.responsavel`, "CS" column shows `cup_contratos.csResponsavel`. Contracts page displays AOV Médio (Average Order Value = LTV ÷ LT) as a KPI, calculated from unique clients in filtered contracts.

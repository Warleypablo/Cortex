...
export function TabelaLtLtv({
  ...
  // estado global → grupo: ambos=total, ativo=ativo, cancelado=cancelado.
  const estadoGrupo = estado === "ambos" ? "total" : estado;
  const grupoDe = (modelo: string): Grupo | undefined =>
    grupos.find((g) => g.modelo === modelo && g.estado === estadoGrupo);

import { SecondaryCard } from "../relatorio-mensal/SlideComponents";
import type { Qoq, TrendPoint } from "./types";
import { useCountUp } from "./useCountUp";

// ─────────────────────────────────────────────────────────────────────────────
// Peças compartilhadas pelos dois slides "Visão do Trimestre" (Recorrente e
// Pontual). Extraídas para que as duas telas não divirjam em formatação,
// animação ou na semântica do badge de variação.
// ─────────────────────────────────────────────────────────────────────────────
...
export function prevQuarterLabel(trimestre: string): string {
...
export function VariacaoBadge({ q }: { q: Qoq }) {
...
/**
 * Sparkline de barras por trimestre: anteriores apagados (zinc), o atual na cor da
 * métrica. Barras entram em cascata (fade + sobe) após o card aparecer.
 * Pontos sem valor (ex.: estoque sem snapshot) são ignorados.
 */
export function QuarterSparkline({
  ...
  const pontos = series.filter((p) => getValue(p) != null);
  if (pontos.length < 2) return null;
  const max = Math.max(...pontos.map((p) => getValue(p) as number));
  if (max <= 0) return null;
  const BAR_H = 52;
  return (
    <div className="flex items-end gap-2 mt-auto pt-3">
      {pontos.map((p, i) => {
        const v = getValue(p) as number;
        const isCurrent = i === pontos.length - 1;
...
export function tileAnim(delayMs: number) {
  return {

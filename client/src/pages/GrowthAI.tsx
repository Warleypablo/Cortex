...
const SUGGESTION_CARDS = [
  {
    icon: BarChart3,
    text: "Análise completa do mês atual",
    color: "text-emerald-500 dark:text-emerald-400",
    bg: "bg-emerald-50 dark:bg-emerald-950/30",
  },
  {
    icon: Activity,
    text: "Health Check — diagnóstico de Growth",
    color: "text-red-500 dark:text-red-400",
    bg: "bg-red-50 dark:bg-red-950/30",
  },
  {
    icon: TrendingUp,
    text: "Top campanhas por performance",
    color: "text-blue-500 dark:text-blue-400",
    bg: "bg-blue-50 dark:bg-blue-950/30",
  },
  {
    icon: GitBranch,
    text: "Comparar funis de vendas",
    color: "text-purple-500 dark:text-purple-400",
    bg: "bg-purple-50 dark:bg-purple-950/30",
  },
  {
    icon: DollarSign,
    text: "CPL e CPA por campanha",
    color: "text-amber-500 dark:text-amber-400",
    bg: "bg-amber-50 dark:bg-amber-950/30",
  },
  {
    icon: Users,
    text: "MQL vs Não-MQL este mês",
...
export default function GrowthAI() {
  usePageTitle("Growth AI");
  useSetPageInfo("Growth AI", "Analista inteligente de Growth Marketing");
  ...
  const { data: conversas = [] } = useQuery<Conversa[]>({
    queryKey: ["/api/growth-ai/conversas"],
    queryFn: async () => {
      const res = await fetch("/api/growth-ai/conversas", {
        credentials: "include",
      });
  ...
  const { data: mensagens = [] } = useQuery<Mensagem[]>({
    queryKey: [`/api/growth-ai/conversas/${activeConversaId}/mensagens`],
    enabled: !!activeConversaId,
    queryFn: async () => {
      const res = await fetch(
        `/api/growth-ai/conversas/${activeConversaId}/mensagens`,
        { credentials: "include" }
      );
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await fetch("/api/growth-ai/chat", {
        method: "POST",
    ...
    onSuccess: (data) => {
      setOptimisticMessages((prev) => [
        ...prev,
        { role: "assistant", conteudo: data.conteudo },
      ]);
      queryClient
        .invalidateQueries({
          queryKey: [
    ...
    onError: (error) => {
      setOptimisticMessages((prev) => [
        ...prev,
        { role: "assistant", conteudo: `Erro: ${error.message}` },
      ]);
      setIsLoading(false);
    },
  });
  ...
  const deleteConversaMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await fetch(`/api/growth-ai/conversas/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
            ...
            {conversas.length === 0 ? (
              <div className="text-center py-8 px-4">
                ...
                <p className="text-xs text-gray-400 dark:text-zinc-600 mt-1">
                  Inicie uma nova conversa com o Growth AI
                </p>
              </div>
            ) : (
          ...
          <>
            ...
            <div className="border-b border-gray-200 dark:border-zinc-800 px-4 py-2 flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400 dark:text-zinc-500">
                Analista inteligente de Growth Marketing
              </span>
            </div>
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                ...
                {isLoading && (
                  <div className="flex gap-3">
                    ...
                    <div className="bg-gray-100 dark:bg-zinc-800 rounded-2xl px-4 py-3 text-emerald-600 dark:text-emerald-400">
                      <TypingIndicator />
                    </div>
                  </div>
                )}
            ...
            <div className="border-t border-gray-200 dark:border-zinc-800 p-4">
              <div className="max-w-3xl mx-auto flex gap-3 items-end">
                <Textarea
                  ...
                  placeholder="Pergunte sobre Growth Marketing..."
                  disabled={isLoading}
                ...
                <Button
                  ...
                  size="icon"
                  className="h-11 w-11 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white flex-shrink-0"
                >
                  {isLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8">
            <div className="max-w-2xl w-full text-center">
              ...
              <p className="text-gray-500 dark:text-zinc-400 mb-8">
                Analista inteligente de Growth Marketing. Pergunte sobre
                campanhas, funis, CPL, leads e muito mais.
              </p>
                ...
                {SUGGESTION_CARDS.map((card, index) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-start gap-3 p-4 rounded-xl border border-gray-200 dark:border-zinc-800 hover:border-emerald-300 dark:hover:border-emerald-800 bg-white dark:bg-zinc-900 hover:bg-emerald-50/50 dark:hover:bg-emerald-950/20 transition-colors text-left"
                    >
                      <div
              ...
              <p className="text-xs text-gray-400 dark:text-zinc-600 mt-8">
                As respostas sao geradas por inteligencia artificial com acesso
                aos dados de Growth Marketing.
              </p>
            </div>
          </div>
        )}

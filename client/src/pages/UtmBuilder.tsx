...
import { Copy, Check, AlertTriangle, Link2, History, Settings, Loader2, Plus, ShieldAlert, Sparkles, PlayCircle } from "lucide-react";
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      <Tabs defaultValue="gerar" className="w-full">
        <TabsList className="mb-6">
          ...
          <TabsTrigger value="tutorial" data-testid="tab-tutorial">
...
// Auto-slug do encurtador — padrão {plataforma}-[pessoa]-{destino|apelido}.
// Turbo (organic) omite a pessoa; sócios levam o nome. Cast = youtube-cast → ytcast.
const SLUG_PLATFORM: Record<string, string> = {
  instagram: "ig",
  tiktok: "tt",
  youtube: "yt",
  "youtube-cast": "ytcast",
...
const SLUG_PERSON: Record<string, string> = {
  victor: "victor",
  andre: "andre",
  rodrigo: "rodrigo",
};
function buildAutoSlug(source: string, medium: string, tail: string): string {
  if (!source) return "";
  const plat = SLUG_PLATFORM[source] || source;
  const person = SLUG_PERSON[medium] || ""; // organic (Turbo) e demais → sem pessoa
  return [plat, person, tail].filter(Boolean).join("-");
}
...
function TabGerar() {
  ...
  const [slugEdited, setSlugEdited] = useState(false); // true = usuário digitou; para de auto-sugerir
  ...
  useEffect(() => {
    ...
    setTermOther("");
    setSlug("");
    setSlugEdited(false); // volta a auto-sugerir no novo medium
  }, [medium]);
  ...
  // Auto-slug: {plataforma}-[pessoa]-{content}. O content é o destino (bio) ou o apelido (post).
  const suggestedSlug = useMemo(
    () => buildAutoSlug(source, medium, finalContent),
    [source, medium, finalContent],
  );
  // Enquanto o usuário não editar o slug à mão, ele acompanha a sugestão.
  useEffect(() => {
          ...
          <div className="space-y-3 pt-4 border-t">
            ...
            <div className="space-y-1.5">
              <Label className="flex items-center gap-1.5">
                ...
                <span className="text-muted-foreground font-normal text-xs">(sugerido automático — pode editar; vazio gera aleatório)</span>
              </Label>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Input
                  ...
                  placeholder="ig-victor-criativos-ugc"
                  value={slug}
                  onChange={(e) => {
                    setSlugEdited(true);
                ...
                />
                {slugEdited && suggestedSlug && slug !== suggestedSlug && (
                  <Button
                    type="button"
                    size="sm"
                    variant="ghost"
              ...
              <p className="text-xs text-muted-foreground">
                Padrão: <span className="font-mono">{`{plataforma}-{pessoa}-{destino ou apelido}`}</span>. Turbo omite a pessoa. Ex: <span className="font-mono">yt-victor-5-coisas-ugc</span>.
              </p>
            </div>
...
// ============================================================================
// ABA — TUTORIAL (vídeo Loom embutido)
// ============================================================================

// Cole aqui a URL de share do Loom (ex: "https://www.loom.com/share/abc123").
// Deixe "" pra exibir o placeholder. A conversão pra /embed é automática.
...
function TabTutorial() {
  return (
    <Card>
      <CardContent className="p-6 space-y-4">
        <div>
          <h2 className="text-2xl font-bold mb-1">Como usar o UTM Builder</h2>
          <p className="text-sm text-muted-foreground">
            Gere todo link aqui e divulgue sempre o <strong>link curto</strong>{" "}
            (<code className="font-mono">marketing.turbopartners.com.br/...</code>) — é ele que faz
            clique, MQL e venda aparecerem no Histórico.
          </p>
        </div>

        {LOOM_SHARE_URL ? (
          <div className="relative w-full rounded-lg overflow-hidden border border-gray-200 dark:border-zinc-700" style={{ paddingBottom: "56.25%" }}>
            <iframe
              src={toLoomEmbed(LOOM_SHARE_URL)}
              title="Tutorial UTM Builder"
              allowFullScreen
          ...
          <div className="flex flex-col items-center justify-center text-center gap-2 rounded-lg border-2 border-dashed border-gray-300 dark:border-zinc-700 bg-muted/40 py-16 px-6">
            <PlayCircle className="w-10 h-10 text-muted-foreground" />
            <p className="text-sm font-medium">Vídeo em breve</p>
            <p className="text-xs text-muted-foreground max-w-sm">
              Assim que o tutorial for gravado, ele aparece aqui.
            </p>
          </div>
        )}
      </CardContent>

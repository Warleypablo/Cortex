/**
 * Passo 2/3 — Sobe pro Gerenciador os 12 vídeos do lote "Areia Movediça"
 * (Victor Peixoto, single-format 9x16). Download do Drive → metaUploadVideo (chunked).
 * Título no Meta = `${base}_9x16`. IDEMPOTENTE (descobre por nome, pula). Retry por vídeo p/ flakiness.
 *
 *   npx tsx scripts/ads/subir-areia-movedica-upload.ts        # DRY
 *   npx tsx scripts/ads/subir-areia-movedica-upload.ts --go   # baixa e sobe
 */
...
import { ITEMS, LOTE, metaTitle } from "./areia-movedica.data";
...
(async () => {
  ...
  for (const it of ITEMS) want.set(normName(metaTitle(it.base)), it);

  console.log(`Lote "${LOTE}" · ${ITEMS.length} vídeos single-format · conta ${ACC}`);
  ...
  const toUpload = ITEMS.filter((it) => !found.has(normName(metaTitle(it.base))));
  ...
  for (let i = 0; i < toUpload.length; i++) {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          videoId = await withBackoff(`upload ${title}`, () => metaUploadVideo(ACC, `${title}.mp4`, buffer), { max: RL_MAX_RETRIES, log: (m) => console.log(`   ${m}`) });
          break;
        } catch (e) {
          if (attempt < 3 && isTransientUpload(e)) { console.log(`   ⏳ upload flaky — retry ${attempt}/2 em 12s`); await sleep(12000); continue; }
          throw e;
        }
      }
      uploaded.push({ title, videoId: videoId! });

/**
 * Sobe os ads do lote "3 - Creators Summit - Empresário" na camp QUENTE do Summit
 * ([TP] [Vendas] [Quente] [CBO] [Summit] - Creators Summit ES).
 * 6 hooks (Victor h1-3 = TP1745-1747, Lucas h1-3 = TP1748-1750) → 1 CONJUNTO POR AVATAR.
 * Cada hook = 1 ad PAREADO 9x16(stories)+4x5(feed) via asset_feed_spec. Tudo PAUSED.
 *
 * Config + copy/link/CTA/UTM clonados do conjunto '10' (120252239374870450) da MESMA campanha
 * (copy padrão do Summit + link creators-summit-es). Match de vídeo ESTRITO pelo nome EXATO.
 *
 *   npx tsx scripts/ads/subir-summit-empresario-ads.ts        # DRY (descobre + mostra plano)
 *   npx tsx scripts/ads/subir-summit-empresario-ads.ts --go   # cria
 */
...
const CAMP = "120251818147660450"; // [TP] [Vendas] [Quente] [CBO] [Summit] - Creators Summit ES
const CLONE_ADSET = "120252239374870450"; // conjunto '10' (config + copy/link do Summit)
const TEMA = "Creators Summit Empresário";
...
const PERSONAS = [
  { persona: "Victor", from: 1745, to: 1747 },
  { persona: "Lucas", from: 1748, to: 1750 },
];
  ...
  return {
    ...
    asset_feed_spec: {
      ...
      asset_customization_rules: [
        // IG-only (o conjunto '10' entrega só no Instagram). 9x16 → story+reels; 4x5 → resto.
...
(async () => {
  // 1) Biblioteca TP1745-1750 (base = nomeDrive)
  ...
  // 3) copy/link/UTM/page/ig do irmão (conjunto '10')
  ...
  console.log(`\nCampanha QUENTE Summit ${CAMP}  ·  ${TEMA}  ·  ${plans.length} conjuntos (1 por avatar)`);
  console.log(`Copy clonada do conjunto '10' (${reused.message.length} chars): "${reused.message.slice(0, 90).replace(/\n/g, " ")}…"`);
  ...
  console.log(`\nResumo: ${createdSets} conjunto(s) + ${createdAds} ad(s) criado(s) nesse run.`);

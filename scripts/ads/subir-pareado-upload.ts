/**
 * Passo 2/3 GENÉRICO — sobe os vídeos PAREADOS (9x16 + 4x5) de um lote (--data).
 * Download do Drive → metaUploadVideo chunked. Título = `${base}_9x16` / `_4x5`.
 * IDEMPOTENTE (descobre por nome, pula). Retry por vídeo p/ flakiness.
 *
 *   npx tsx scripts/ads/subir-pareado-upload.ts --data ./scripts/ads/ugc-victor.data.ts        # DRY
 *   npx tsx scripts/ads/subir-pareado-upload.ts --data ./scripts/ads/ugc-victor.data.ts --go    # sobe
 */
...
(async () => {
  ...
  const { PAIRS, LOTE, metaTitle9, metaTitle4 } = D;
  const ITEMS = PAIRS.flatMap((p: any) => [{ title: metaTitle9(p.base), driveId: p.drive9x16 }, { title: metaTitle4(p.base), driveId: p.drive4x5 }]);
  ...
  console.log(`Lote "${LOTE}" · ${ITEMS.length} vídeos (${PAIRS.length} pares) · conta ${ACC}`);
  ...
  const toUpload = ITEMS.filter((it: any) => !found.has(normName(it.title)));
  ...
  for (let i = 0; i < toUpload.length; i++) {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try { videoId = await withBackoff(`upload ${it.title}`, () => metaUploadVideo(ACC, `${it.title}.mp4`, buffer), { max: RL_MAX_RETRIES, log: (m) => console.log(`   ${m}`) }); break; }
        catch (e) { if (attempt < 3 && isTransientUpload(e)) { console.log(`   ⏳ upload flaky — retry ${attempt}/2 em 12s`); await sleep(12000); continue; } throw e; }
      }
      uploaded.push({ title: it.title, videoId: videoId! });
      console.log(`   ✅ video_id=${videoId}`);
    } catch (e) { failed.push(`${it.title}: ${e instanceof Error ? e.message : e}`); console.log(`   ⛔ FALHOU: ${e instanceof Error ? e.message : e}`); }
  }
  ...
  if (uploaded.length) {
    try {
      ...
      res.forEach((r: any, i: number) => { const b = typeof r?.body === "string" ? JSON.parse(r.body) : r?.body; console.log(`  ${uploaded[i].title} → ${b?.status?.video_status ?? "?"}`); });
    } catch (e) { console.log(`(status batch falhou: ${e instanceof Error ? e.message : e})`); }
  }

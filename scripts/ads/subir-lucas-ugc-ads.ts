/**
 * Sobe os ads do lote "Lucas - UGC x Anuncios" (TP1771-1792) na campanha CBO QUENTE de
 * Creators ([TP] [Leads] [CBO] [QUENTE] [Creators] - Campanha de conversão), padrão Turbo:
 * 4 conjuntos (b1 h01-05, b1 h06-11, b2 h01-05, b2 h06-11 — split 5+6 aprovado pelo Caio),
 * ads pareados 9x16+4x5.
 *
 ...
 * - Criação PAUSED e idempotente; --activate ativa SÓ os ads (conjunto NUNCA — é manual do Caio)
 *
 *   npx tsx scripts/ads/subir-lucas-ugc-ads.ts             # DRY (descobre + mostra plano)
 *   npx tsx scripts/ads/subir-lucas-ugc-ads.ts --go        # cria (PAUSED)
 *   npx tsx scripts/ads/subir-lucas-ugc-ads.ts --activate  # ativa SÓ os ads
 */
...
const TEMA = "UGC x Anuncios"; // singular — segue o nome dos arquivos do Lucas
const PERSONA = "Lucas";
const TP_START = 1771;
const TP_END = 1792;
const EXPECTED_ADS = 22;
...
(async () => {
  ...
  // ---------- MODO ATIVAÇÃO (só ads; conjunto NUNCA — regra do Caio) ----------
  if (activate) {
    if (!loteSets.length) throw new Error(`Nenhum conjunto "${loteSufixo}" na campanha — rode com --go antes.`);
    console.log(`\nAtivando ADS do lote "${loteSufixo}": ${loteSets.length} conjunto(s)...`);
    let adsOn = 0;
  ...
  if (rows.length !== EXPECTED_ADS) throw new Error(`Esperava ${EXPECTED_ADS} linhas TP${TP_START}-TP${TP_END}, achei ${rows.length}`);
  ...
  // 5) monta 4 conjuntos: por body, split h01-05 / h06-11 (5+6 aprovado)
  ...
  if (!go) { console.log("(DRY) Rode com --go pra criar; depois --activate pra ativar SÓ os ads."); process.exit(0); }
  ...
  if (totalAds !== EXPECTED_ADS) throw new Error(`esperava ${EXPECTED_ADS} ads, montei ${totalAds} — revisar match antes de criar`);

  // 6) cria (PAUSED — e conjunto FICA PAUSED, quem liga é o Caio)
  ...
  console.log(`\nResumo: ${createdSets} conjunto(s) + ${createdAds} ad(s) criados (PAUSED). Rode com --activate pra ativar SÓ os ads.`);

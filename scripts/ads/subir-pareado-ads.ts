/**
 * Passo 3/3 GENÉRICO — cria os conjuntos + ads PAREADOS (9x16+4x5) de um lote (--data)
 * na campanha CBO JÁ EXISTENTE do funil. Split por GROUPS. Config + COPY clonados de CLONE_ADSET.
 * Conjuntos SEM budget (CBO). fixTargeting (par explore/explore_home). Tudo PAUSED.
 *
 *   npx tsx scripts/ads/subir-pareado-ads.ts --data ./scripts/ads/ugc-victor.data.ts        # DRY
 *   npx tsx scripts/ads/subir-pareado-ads.ts --data ./scripts/ads/ugc-victor.data.ts --go     # cria
 */
...
(async () => {
  ...
  const { PAIRS, CAMPAIGN, CLONE_ADSET, NN_FLOOR, GROUPS, conjName } = D;
  ...
  console.log(`Descoberta: ${pagesRead} página(s) · achou todos: ${foundAll ? "sim" : "NÃO"}`);
  ...
  const copy: ReusedCopy = {
    message: afs.bodies?.[0]?.text || vd.message || "", title: afs.titles?.[0]?.text || vd.title,
  ...
  for (const c of plan) {
    ...
    if (!adsetId) {
      ...
      adsetId = created.id; totalCreated++;
  ...
  console.log(`\nResumo: ${plan.reduce((s, c) => s + c.ads.length, 0)} ad(s) no plano${allErrors.length ? ` · ${allErrors.length} erro(s)` : ""} (tudo PAUSED).`);
